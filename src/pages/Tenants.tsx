...
function TenantDialog({ userId, onCreated }: { userId: string; onCreated: () => void }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      // Get all units that are either vacant or occupied but unassigned
      const { data, error } = await supabase
        .from("units")
        .select("id,house_number,unit_type,floor_level,rent,status,properties(name),leases(id)")
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto">
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogHeader>
          ...
          <DialogDescription>Minimal details — full name, phone, and unit assignment.</DialogDescription>
        </DialogHeader>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 border-t pt-4">
            <div className="space-y-3">
              <Label className="font-semibold text-sm">Assign available unit</Label>
